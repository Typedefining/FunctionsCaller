...
namespace
{
	...
	FCValue evaluateIf(const FCIfExprAST *node, FCEvaluationContext &context)
	{
		...
		if (condition.type == FCValueCategory::Integer)
			...
			return makeDangleValue();
		return evaluateExpression(truthy ? node->getThen() : node->getElse(), context);
	}
	...
	FCValue evaluateDeclaration(const FCVarDeclExprAST *node, FCEvaluationContext &context)
	{
	...
	FCValue evaluateProgram(const FCProgramAST *node, FCEvaluationContext &context)
	{
					   ...
					   FCEvaluationContext &context)
{
	...
	if (lhs.type == FCValueCategory::Floating && rhs.type == FCValueCategory::Floating)
	{
		...
		{
		case '+':
			result.evaluteVal.doubleVal = lhs.evaluteVal.doubleVal + rhs.evaluteVal.doubleVal;
			break;
		case '-':
			result.evaluteVal.doubleVal = lhs.evaluteVal.doubleVal - rhs.evaluteVal.doubleVal;
			break;
...
FCValue evaluateExpression(
	const FCExprAST *expression,
	FCEvaluationContext &context)
{
	if (!expression)
		return makeDangleValue();
	...
	if (auto *node =
			dynamic_cast<const FCNumberExprAST *>(expression))
		return evaluateNumber(node);

	if (auto *node =
			dynamic_cast<const FCStringExprAST *>(expression))
		return makeStringValue(node->m_stringVal);

	if (auto *node =
			dynamic_cast<const FCVariableExprAST *>(expression))
		return evaluateVariable(node, context);

	if (auto *node =
			dynamic_cast<const FCBinaryExprAST *>(expression))
		return evaluateBinary(node, context);

	if (auto *node =
			dynamic_cast<const FCCallExprAST *>(expression))
		return evaluateCall(node, context);

	if (auto *node =
			dynamic_cast<const FCIfExprAST *>(expression))
	...
	if (auto *node =
			dynamic_cast<const FCVarDeclExprAST *>(expression))
		return evaluateDeclaration(node, context);

	if (auto *node =
			dynamic_cast<const FCProgramAST *>(expression))
		return evaluateProgram(node, context);

	if (auto *node =
			dynamic_cast<const FCFunctionAST *>(expression))
	{
		context.functions.registerFunction(
			const_cast<FCFunctionAST *>(node));
		return makeDangleValue();
	}
